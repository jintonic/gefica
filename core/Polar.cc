...
{//need update
   ...
   double r=fC1[idx];
   double tmp = (-2*density/epsilon*(h1+h4)*(h2+h3)
       +1/2/r*(Pxp1-Pxm1)*(h1+h4)
       +Pxp1/h3+Pxm1*h2+Pyp1/h4/r/r+Pym1/r/r/h1)
       /(1/h2+1/h3+1/h4/r/r+1/h1/r/r);
...
void Polar::CreateGridWithFixedStepLength(double steplength)
{
   ...
   for (int i=0;i<n;i++) {
      if(i>n1-1)fC2[i]=fC2[i-n1]*2*3.1415926/n2;
      else fC2[i]=0;
      if(i%n1==0)fC1[i]=0;
