...
bool X::CalculateField(EMethod method)
{
  ...
  Impuritystr2tf();
   ...
   while (cnt++<MaxIterations) {
      double XUpSum=0;
      double XDownSum=0;
      CreateTask(0,n,&XUpSum,&XDownSum);
...
void X::CreateTask(int begin,int number,double *XUpSum,double *XDownSum)
{
  double Xup1=0,Xdown1=0;
  double Xup2=0,Xdown2=0;
  if(number!=1)
  {
    std::thread t1(&X::CreateTask,this,0+begin,number/2,&Xup1,&Xdown1);
    thread t2(&X::CreateTask,this,begin+number/2,number-number/2,&Xup2,&Xdown2);
   // t2.join();
    t1.join();
    *XUpSum=Xup1+Xup2;
    *XDownSum=Xdown1+Xdown2;
  }
  else
  {
    int i=begin;
